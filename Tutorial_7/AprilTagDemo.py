# Authors: Kent Chesley
# Date: 10/21/23
#
# Adapted from: https://github.com/pupil-labs/apriltags/blob/main/src/pupil_apriltags/bindings.py.
#
# This program puts bounding boxes on AprilTags in photos provide by pupil-labs and outputs related information.
# 
# Must first add pupil-apriltags to lab_env with "pip install pupil-apriltags"
#
# Must first add yaml library to lab_env with "pip install pyyaml". Reference: 
# https://github.com/microsoft/pyright/issues/53
#
# Needed to copy the following into this file's parent directory (which is part of our git repo):
# - lib (obtained from \lab_env\Lib\site-packages\pupil_apriltags.)
# - pupil_apriltags.libs (obtained from \lab_env\Lib\site-packages.)
# - test (obtained from https://github.com/pupil-labs/apriltags. Had to downolad the entire repository as a zip
# folder and then copy the test directory out of it.)
...
if __name__ == "__main__":  # noqa
    ...
    os.add_dll_directory(Path(__file__).parent / "pupil_apriltags.libs")
    # Reference: Answer by Fiiila at https://github.com/pupil-labs/apriltags/issues/38.
    at_detector = Detector(
        #searchpath=["apriltags/lib", "apriltags/lib64"],
        # Commented to eliminate error message.
        families="tag36h11",
        nthreads=1,
        quad_decimate=1.0,
        quad_sigma=0.0,
    ...
    with open(test_images_path + "/test_info.yaml") as stream:
        parameters = yaml.safe_load(stream) # changed "load" to "safe_load" to avoid errors. Reference: 
