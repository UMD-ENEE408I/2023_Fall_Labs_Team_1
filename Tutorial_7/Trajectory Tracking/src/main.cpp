...
void limacon(float t, float a, float& x, float & y){
  x = (a+2*a*cos(t))*cos(t);
  y = (a+2*a*cos(t))*sin(t);
}
...
void loop() {
  ...
  while (true) {
    ...
    limacon(leminscate_t_scale * t, leminscate_a, x, y);
